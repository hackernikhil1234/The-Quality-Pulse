...
export default function Sidebar({ className = '' }) {
  ...
  return (
    // 1. MAIN CONTAINER
    <aside
      className={`w-72 h-screen sticky top-0 flex flex-col shadow-2xl z-50 transition-all duration-300 
    ...
    >
      ...
      {/* --- LOGO HEADER --- */}
      <div
        className="h-20 flex items-center gap-4 px-6 relative transition-colors duration-300
        bg-white dark:bg-slate-900 
        ...
        {menuItems.map((group, groupIndex) => {
          ...
          return (
            <div key={groupIndex}>
              {/* Category Label */}
              <h3
                className="px-3 text-[10px] font-bold uppercase tracking-[0.2em] mb-3 font-mono border-l-2 pl-3
                  ...
                  return (
                    <li key={item.path}>
                      ...
                      >
                        ...
                        {/* Label */}
                        <span
                          className={`text-sm font-medium tracking-wide ${active ? 'font-bold' : ''}`}
                        >
                          {item.label}
                        </span>
