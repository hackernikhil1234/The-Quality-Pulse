...
function LocationMarker({ formData, setFormData, setAddressSearch }) {
  useMapEvents({
    click(e) {
...
export default function EditSite() {
  ...
  useEffect(() => {
    if (formData.country) {
      setMapLoading(true);
      fetch(
        `https://nominatim.openstreetmap.org/search?country=${encodeURIComponent(formData.country)}&format=json&addressdetails=1&limit=100`
      )
        .then((res) => res.json())
        .then((data) => {
  ...
  const handleSelectCity = (city) => {
    ...
    fetch(
      `https://nominatim.openstreetmap.org/search?city=${encodeURIComponent(city)}&country=${encodeURIComponent(formData.country)}&format=json&limit=1`
    )
      .then((res) => res.json())
      .then((data) => {
        if (data.length > 0) {
  ...
  const selectedEngineers = formData.assignedEngineers
    .map((id) => engineers.find((e) => e._id === id))
    .filter(Boolean);
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400">
                    Update site details and description
                  </p>
                </div>
              </div>
                    ...
                    <div className="relative" ref={cityDropdownRef}>
                      <div className="relative">
                        <input
                          ...
                          className="w-full px-4 py-3 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg focus:outline-none focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20 dark:focus:ring-yellow-500/30 transition-all duration-200"
                          placeholder={
                            mapLoading ? 'Loading cities...' : 'Type to search cities...'
                          }
                          required
              ...
              <div className="space-y-4">
                ...
                <div className="h-96 rounded-lg overflow-hidden border-2 border-slate-300 dark:border-slate-600 shadow-lg">
                  ...
                  >
                    ...
                    />
                    <MapCenter coordinates={formData.coordinates} />
                    <LocationMarker
                      formData={formData}
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 text-center pt-2">
                  💡 Click anywhere on the map to update the location. The address will be
