...
export default function AuditLog() {
  ...
  const getActionColor = (action) => {
    ...
    if (lowerAction.includes('created') || lowerAction.includes('added')) return 'text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20 px-2 py-0.5 rounded';
    if (lowerAction.includes('updated') || lowerAction.includes('modified')) return 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-2 py-0.5 rounded';
    if (lowerAction.includes('deleted') || lowerAction.includes('removed')) return 'text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-0.5 rounded';
    if (lowerAction.includes('rejected')) return 'text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20 px-2 py-0.5 rounded';
    return 'text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-900/50 px-2 py-0.5 rounded';
  };

  const getResourceIcon = (type) => {
    switch (type?.toLowerCase()) {
      case 'user': return <FiUser className="inline mr-2" />;
      case 'site': return <FiActivity className="inline mr-2" />;
      case 'report': return <FiMonitor className="inline mr-2" />;
      case 'auth': return <FiClock className="inline mr-2" />;
      default: return <FiDatabase className="inline mr-2" />;
    }
  };
        ...
        <main className="p-4 md:p-8 space-y-8 animate-in fade-in duration-500">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
             <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Total Trail Events</p>
                <p className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter">
                  {totalLogs.toLocaleString()}
                </p>
             </div>
             <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Events This Page</p>
                <p className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter">{logs.length}</p>
             </div>
          </div>
                      ...
                      {logs.length === 0 ? (
                        ...
                        logs.map((log) => (
                          <tr key={log._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                            <td className="px-6 py-5 whitespace-nowrap">
                              <div className="flex flex-col">
                                ...
                                <span className="text-slate-400 dark:text-slate-500 text-xs flex items-center gap-1">
                                  <FiClock className="text-slate-300" /> {new Date(log.timestamp).toLocaleTimeString()}
                                </span>
                              </div>
                            </td>
                            ...
                            <td className="px-6 py-5">
                              <span className={`text-xs font-black uppercase tracking-wider ${getActionColor(log.action)}`}>
                                {log.action}
                              </span>
                            </td>
