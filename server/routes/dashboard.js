  ...
  try {
    ...
    if (user.role === 'Admin') {
      ...
      const averageResponseTime =
        countWithResponse > 0 ? Math.round(totalResponseHours / countWithResponse) : 24;
  ...
  try {
    ...
    if (user.role === 'Admin') {
      ...
      const reportActivities = reviewedReports.map((report) => ({
        ...
        icon:
          report.status === 'Approved' ? 'check' : report.status === 'Rejected' ? 'times' : 'clock',
        type:
      ...
      const reportActivities = recentReports.map((report) => ({
        ...
        user: user.name,
        timestamp:
          report.status === 'Pending' ? report.createdAt : report.reviewedAt || report.updatedAt,
        icon:
          report.status === 'Approved'
            ? 'check'
            : report.status === 'Rejected'
      ...
      activities = [...reportActivities, ...assignmentActivities, ...profileUpdates]
        .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))
        .slice(0, 10);
    }
...
router.get('/recent-reports', protect, async (req, res) => {
  try {
    ...
    const recentReports = await Report.find({
      ...
      .select(
        'title status createdAt site testResult complianceStatus materialTested reviewedAt reviewNotes'
      )
